...
def settlement(subset):
	...
	for i in numpy.arange(xi,xf,1):
		for j in numpy.arange(yi,yf,1):
			x = 0.25*(random.random()+1)
			y = 0.4*(random.random()+0.5)
			z = 0.35*(random.random()+1)
			array.append(T([1,2])([i-x-0.1,j])(CUBOID([x,y,z])))
		
		return array

complex_home = STRUCT(settlement([-3,0,0.1,7]))


model_complete = STRUCT([forests, lake, model,complex_home])
